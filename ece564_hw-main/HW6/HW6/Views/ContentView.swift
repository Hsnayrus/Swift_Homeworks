...
struct ContentView: View {
    ...
    @StateObject var currentPerson: PersonObs = PersonObs(DukePerson())
    @Environment(\.editMode) private var editMode
    @State private var disableTextField: Bool = false
    @State var showBack: Bool = false
    @State var audioPlayer: AVAudioPlayer!
    @State var triggerSound: AVAudioPlayer!
    ...
    init(person newPerson: DukePerson){
        ...
        //        currentPerson.updatePerson(contentPerson)
    }
        ...
        ZStack {
            ...
            VStack {
                ...
                if(!showBack){
                            ...
                            VStack(alignment: .leading){
                                ...
                                VStack{
                                    HStack{
                                        ...
                                        .font(.title)
                                        .foregroundColor(Color.black)
                                        .padding(/*@START_MENU_TOKEN@*/.all/*@END_MENU_TOKEN@*/).overlay(RoundedRectangle(cornerRadius: 20).stroke(Color.white, lineWidth: 1))
                                    }
                                    Text(description)
                                        .font(.title2)
                                        .foregroundColor(Color(hue: 0.225, saturation: 1.0, brightness: 1.0))
                    ...
                    /*
                     In reference with : https://medium.com/swift-productions/swiftui-play-an-audio-with-avaudioplayer-1c4085e2052c
                     */
                    if(netID != "sj346") {
                        VStack {
                            Text("Play Music")
                        ...
                        .transition(.slide)
                        .onAppear {
                            let anotherSound = Bundle.main.path(forResource: "FiringSound", ofType: "mp3")
                            if(anotherSound != nil){
                                self.triggerSound = try! AVAudioPlayer(contentsOf: URL(fileURLWithPath: anotherSound!))
                                self.triggerSound.enableRate = true
                            }
                        }
                    }
                    Spacer()
