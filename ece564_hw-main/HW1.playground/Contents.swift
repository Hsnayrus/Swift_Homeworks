...
class DukePerson: Person, ECE564, CustomStringConvertible{
    ...
    var description: String{
        get{
            ...
            if (role == .Other){
                return "\(firstName) \(lastName) is from \(whereFrom) and \(pronouns) is/are not related to Duke "

            }
            else{
                return "\(firstName) \(lastName) is from \(whereFrom) and \(pronouns) is/are a  \(role) at Duke"
            }
...
class HW1ViewController : UIViewController{
    ...
    func updateAttributes(person newPerson: DukePerson, attribute newAtr: String){
        
        if(newAtr == "Male"){
            newPerson.gender = .Male
        }
        else if(newAtr == "Female"){
            newPerson.gender = .Female
        }
        else if(newAtr == "Non-binary"){
            newPerson.gender = .NonBinary
            
        }
        else if(newAtr == "Professor"){
            newPerson.role = .Professor
        }
        else if(newAtr == "Student"){
            newPerson.role = .Student
        }
        else if(newAtr == "Teaching Assistant"){
            newPerson.role = .TA
            
        }
        else{
            newPerson.role = .Other
        }
    }
    
    func newSegment(titles newTitles: [String], rect newRect: CGRect, person newPerson: DukePerson) -> UISegmentedControl {
        ...
        for i in newTitles{
            newAction = UIAction(title: "\(i)"){ _ in self.updateAttributes(person: newPerson, attribute: i)}
    ...
    override func loadView() {
        ...
        let view = UIView()
        let textDisplay: String = newPerson.description
...
//        //Max count of description is 655 characters
        ...
        let genderSegment = newSegment(titles: genders, rect: genderRect,person: newPerson )
        
        //Duke Role Segment
        let dukeRoleRect: CGRect = CGRect(x: 7.5, y: 335, width: 360, height: 50)
        let dukeRoles: [String] = ["Student", "Professor", "TA", "Other"]
        let dukeRolesSegment = newSegment(titles: dukeRoles, rect: dukeRoleRect, person: newPerson)
